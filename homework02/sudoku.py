...
def find_possible_values(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.Set[str]:
    ...
    occupied = {val for val in row_vals + col_vals + block_vals if val != "."}
    all_values = {str(i) for i in range(1, 10)}
    return all_values - occupied
...
def solve(grid: tp.List[tp.List[str]]) -> tp.Optional[tp.List[tp.List[str]]]:
    ...
    if empty_pos is None:
        return grid
    row, col = empty_pos  # Распаковываем позицию
...
def generate_sudoku(N: int) -> tp.List[tp.List[str]]:
    ...
    for block in range(3):
        shuffled = random.sample(numbers, 9)
        start_row = block * 3
        start_col = block * 3
        idx = 0
        for i in range(3):
            for j in range(3):
                empty_grid[start_row + i][start_col + j] = shuffled[idx]
                idx += 1
    solved = solve(empty_grid)
    if solved is None:
        solved = [
            ["5", "3", "4", "6", "7", "8", "9", "1", "2"],
            ["6", "7", "2", "1", "9", "5", "3", "4", "8"],
